...
const claimNft = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  // 2. Verify the user using web3sdks Auth
  const web3sdksUser = await getUser(req);

  // First, get all the public repos of the web3sdks organization
  ...
  const repos: GithubRepo[] = await fetch(
    `https://api.github.com/orgs/web3sdks/repos?per_page=100`
  ).then((res) => res.json());

  if (!session || !web3sdksUser) {
  ...
  for (const repo of reposToCheck) {
    const contributors: GithubContributor[] = await fetch(
      `https://api.github.com/repos/web3sdks/${repo}/contributors`,
      {
